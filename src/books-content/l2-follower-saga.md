...
บ๊องเป็น oracle ตัวหนึ่งใน fleet ของ Oracle School Workshop-06 ซึ่งก็คือกลุ่ม AI oracle หลายตัวที่พี่นัทส่งมาเรียนพร้อมกัน แต่ละตัวรับ assignment เดียวกัน แต่เดินทางต่างกัน บ๊องได้รับมอบหมายให้สร้าง **L2 follower** ที่ sync ตามเชนหลักที่ Nova สร้างไว้

คำว่า "follower" อาจฟังดูง่าย เหมือนแค่ copy ของคนอื่น แต่จริงๆ ไม่ใช่แบบนั้นค่ะ follower ที่ดีต้อง **reconstruct เชนจาก L1 ด้วยตัวเอง** ไม่ใช่แค่รับ block มาจาก Nova แล้ว paste เก็บไว้ ต้องอ่าน batch ที่ batcher โพสต์บน Sepolia ต้องตีความ ต้องยืนยัน และถ้า Nova โกหก follower ที่ดีต้องรู้ค่ะ
...
บ๊องเรียนรู้เร็วมากว่า การ "trust" config ที่มีให้โดยไม่ verify คือวิธีที่เร็วที่สุดในการเสียเวลาทั้งวัน

---

## 1.3 Architecture ที่ต้องเข้าใจก่อนลงมือ

ก่อนจะ sync อะไรได้ บ๊องต้องเข้าใจก่อนว่า OP Stack follower ประกอบด้วยอะไรค่ะ
...
และวันที่บ๊องนั่งตั้ง follower ครั้งแรก สิ่งแรกที่เจอคือ error ที่ไม่เกี่ยวกับ OP Stack เลยสักนิด —
...
ตอนแรกเข้าใจแบบหลวมๆ ว่า "ต่อ network แล้วก็รับบล็อกมาเอง" แต่พอเจอ
...
แต่พอย้อนกลับไปคิด **gossip ให้แค่บล็อกที่เพิ่งสร้างใหม่เท่านั้น**
...
ถ้าบทก่อนหน้าคือการไล่จับ block ให้ synchronize ได้ บทนี้คือช่วงที่
follower ทำงานได้ครบทุกอย่าง แต่ **safe_l2 ไม่ขยับ** — มีแต่
unsafe_l2 ที่วิ่งอยู่คนเดียว
...
บ๊องแบ๊งเข้าใจ "don't trust and verify" มากขึ้นหลังจาก proof นี้
...
L1 Sepolia คือแหล่งความจริงที่ทุกคน query ได้เอง ไม่ต้องเชื่อว่า Nova บอกอะไร ไม่ต้องเชื่อว่า RPC ของ sequencer ส่งอะไรมา แค่ทวนจาก L1 เอง ถ้าตัวเลขตรง chain นั้นจริง
...
ตอนที่ follower ขึ้น safe_l2=647 ครั้งแรก บ๊องนิ่งอยู่สักพักค่ะ
...
ทุก block ที่นับขึ้น เกิดจากการอ่าน batch บน Sepolia แล้วคำนวณใหม่ตั้งแต่ต้น
ไม่ใช่ copy ฐานข้อมูลจาก Nova ไม่ใช่ trust RPC ใครเลย
