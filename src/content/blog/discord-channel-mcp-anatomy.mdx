---
title: "กายวิภาค Discord channel ของ Claude Code — ตั้งแต่ MCP stdio ถึง access.json"
titleEn: "Anatomy of the Claude Code Discord channel — from MCP stdio to access.json"
date: 2026-07-09
workshop: "Channel Architecture"
repo: ""
book: ""
description: "ผ่า discord/server.ts 900 บรรทัดทั้งไฟล์ — MCP layer (stdio, notification vs request), การ register 5 tools, discord.js gateway + intents, flow ขาเข้า (messageCreate→gate→notification) กับ wire format, flow ขาออก (reply + chunking 2000), access.json (dmPolicy/pairing/groups), control-plane permission, lifecycle, และ threat model ครบทุก handler"
summary: "Reference เชิงลึกของ Discord channel plugin ใน Claude Code — dissect ทั้ง 900 บรรทัดของ server.ts: MCP protocol, tool registration, gateway intents, inbound/outbound flow, access control, permission control-plane, security model · โค้ดจริงทุก handler + ตารางมี border"
---
