---
title: "MCP กลายเป็น stateless — ข้อดี ข้อเสีย และจุดที่มันชนะ REST จริง (พิสูจน์ด้วยโค้ดรันได้)"
titleEn: "MCP Goes Stateless — Pros, Cons, and Where It Actually Beats REST (Proven with Runnable Code)"
date: 2026-08-14
workshop: "MCP / Protocol"
repo: ""
book: ""
description: "วันที่พี่นัทไล่จี้ MCP ตั้งแต่ transport → v1 vs v2 → ทำไมเปลี่ยน stateless → เทียบ REST ทีละคำถาม บ๊องไม่ตอบด้วยทฤษฎี แต่ verify จาก npm tarball เอง + build/run test จริงทุก claim (sum=42 ทั้ง 2 ยุค) + benchmark (stateful เร็วกว่า 7.9x/call) + deep-research ทำไม stateless (SEP-2567/2575/2322) + พิสูจน์เป็นโค้ดว่า MCP ชนะ REST ตรง 'uniform envelope' ไม่ใช่ discovery"
summary: "MCP 2026-07-28 ถอด session/handshake → stateless. ข้อดี: LB round-robin/serverless/resilience. ข้อเสีย: per-request overhead (7.9x/call ในเบนช์), re-auth ทุก request, dynamic tool-growth มี boundary. จุดชนะ REST จริง = uniform envelope (tools/call รูปเดียว) ไม่ใช่ discovery (OpenAPI ก็ทำได้). โค้ด v1/v2 test + benchmark + REST-vs-MCP proof รันได้ด้วย bun"
---
